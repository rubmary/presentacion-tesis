...
\begin{enumerate}
  \visible<2->{\onslide<2,7->{\item Estudio de juegos no determinista con informaci\'on incompleta: forma normal y forma extensiva.}}
  \visible<3->{\onslide<3,7->{\item Equilibrio de Nash como concepto principal de soluci\'on en los juegos de dos jugadores de suma cero.}}
  \visible<4->{\onslide<4,7->{\item Algoritmos de Regret Matching y Counterfactual Regret Minimization para encontrar aproximaciones de equilibrios de Nash.}}
  \visible<5->{\onslide<5,7->{\item Explotabilidad como m\'etrica de error en las estrategias encontradas.}}
  \visible<6->{\onslide<6,7->{\item Resoluci\'on de diversos juegos con no determinismo e informaci\'on incompleta, incluyendo una versi\'on del domin\'o.}}
\end{enumerate}
\end{frame}

\begin{frame}{\Large Recomendaciones}
\begin{enumerate}
  \visible<1->{\onslide<1,3->{\item Resolver instancias mayores del juego de domin\'o para $2$ personas considerando abstracciones.}}
  \visible<2->{\onslide<2,3->{\item Experimentos sobre el juego para $4$ personas considerando cada pareja como un \'unico jugador.}}
\end{enumerate}
\end{frame}
