...
\column{.5\textwidth}
\begin{enumerate}
  \visible<10->{\onslide<10,12->{\item La ficha y el domin\'o no se superponen: gana el jugador 1.}}
  \visible<11->{\onslide<11,12->{\item La ficha y el domin\'o s\'i se superponen: gana el jugador 2.}}
\end{enumerate}
}
\column{.45\textwidth}
\visible<10->{%
\begin{tikzpicture}
  ...
  \draw (2, 0) -- (2, 2);
  \draw (3, 0) -- (3, 2);
  \visible<10>{\fill (2.5, 0.5) circle (0.25);}
  \visible<10>{\fill[gray!80] (1.25, 0.25) rectangle (1.75, 1.75);}
  \visible<11->{\fill[gray!80] (0.25, 1.25) rectangle (1.75, 1.75);}
  \visible<11->{\fill (0.5, 1.5) circle (0.25);}
\end{tikzpicture}}
\end{columns}
\vspace{.82cm}
\end{frame}
...
\vspace{.45cm}
\visible<18->{\Alert{Perfect Recall}}
\vspace{.45cm}
...
\begin{enumerate}
  \visible<7->{\onslide<7,17->{\item Historias o nodos.\\Ej: $\emptyset$, $LA, LBb, R$.}}
  \visible<8->{\onslide<8-9,17->{\item Funci\'on que asigna a cada historia (nodo) no terminal un jugador.}}
  \visible<9->{
    \begin{itemize}
      \onslide<9,17->{\item Nodos de azar.}
    \end{itemize}
  }
  \visible<10->{\onslide<10-12,17->{\item Conjuntos de informaci\'on.}}
  \visible<13->{\onslide<13-14,17->{\item Funci\'on que asigna por cada historia (nodo) terminal y cada jugador una utilidad.}}
  \visible<15->{\onslide<15->{\item Distribuci\'on de probabilidad sobre el conjunto de acciones en cada nodo de azar.}}
\end{enumerate}
\end{columns}
\end{frame}
...
\resizebox{.95\textwidth}{!}{\begin{tabular}{lrrrrrc}
    \toprule
    Juego & $N$ & $I$ & Iteraciones & $u(\sigma)$ & $\varepsilon_{\sigma}$ (\%) & Resuelto \\ \midrule
    \onslide<4, 6->{Dudo$(4, 2, 2)$ &    13.107.101 &      327.680 &    2.276.259 &  0,00875 & 0,2382 & \cmark} \\
    Dudo$(5, 2, 2)$ &                   471.858.976 &    7.864.320 &      133.863 & -0,00004 & 1,7695 & \xmark  \\
    \onslide<4, 6->{Dudo$(6, 1, 2)$ &    66.060.163 &    1.769.472 &      543.485 &   -0,597 & 0,5102 & \cmark} \\
    \onslide<4, 6->{Dudo$(6, 2, 1)$ &    66.060.163 &    1.769.472 &      513.786 &    0,597 & 0,6727 & \cmark} \\
    \onslide<4, 6->{Domino$(3, 4)$  & 1.547.645.185 &      104.050 &  365.484.932 &   3.2027 & 0,1812 & \cmark} \\
    \bottomrule
\end{tabular}}
\end{center}}
\end{frame}
...
\begin{enumerate}
  \visible<2->{\onslide<2,7->{\item La forma normal y forma extensiva son modelos adecuados para representar juegos con informaci\'on incompleta y no determinista.}}
  \visible<3->{\onslide<3,7->{\item El equilibrio de Nash es el principal concepto de soluci\'on utilizado para juegos de dos jugadores de suma cero.}}
  \visible<4->{\onslide<4,7->{\item Los algoritmos de Regret Matching y Counterfactual Regret Minimization son efectivos para encontrar una aproximación a un equilibrio de Nash en el tipo de juego mencionado.}}
  \visible<5->{\onslide<5,7->{\item Se utiliz\'o la explotabilidad como m\'etrica para medir la distancia entre una estrategia y un equilibrio de Nash.}}
  \visible<6->{\onslide<6,7->{\item Se resolvieron diversos juegos captados por el modelo, incluyendo una versi\'on del juego de domin\'o.}}
\end{enumerate}
\end{frame}

\begin{frame}{\Large Recomendaciones}
\begin{enumerate}
  \visible<2->{\onslide<2,4->{\item Resolver instancias mayores del juego de domin\'o para $2$ personas considerando abstracciones.}}
  \visible<3->{\onslide<3,4->{\item Experimentos sobre el juego para $4$ personas considerando cada pareja como un \'unico jugador.}}
\end{enumerate}
\end{frame}
