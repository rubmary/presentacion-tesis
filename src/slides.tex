...
\begin{tabular}{@{}cccc@{}}
  ...
  & & & \\
  \onslide<6,10->{\textbf{Ciencias sociales}} &
  \onslide<7,10->{\textbf{Econom\'ia}} &
  \onslide<8,10->{\textbf{Matem\'atica}} &
  \onslide<9,10->{\textbf{Computaci\'on}} \\
\end{tabular}
\end{center}
\end{frame}
...
$\mathcal{R}$ (piedra) & $0,0$ & $-1,1$ & $1,-1$ & \visible<10->{\multirow{3}{*}{\shortstack{Juego de \\ dos jugadores\\de suma cero}}} \\ \cline{2-4}
...
\Alert{Elementos}
\begin{enumerate}
  \visible<7->{\onslide<7, 14->{\item Jugadores.}}
  \visible<8->{\onslide<8, 14->{\item Acciones o estrategias puras: $\mathcal{R}$, $\mathcal{P}$, $\mathcal{S}$.}}
  \visible<9->{\onslide<9, 14->{\item Funci\'on de pago o utilidades.}}
\end{enumerate}
}
\column{.52\textwidth}
\visible<11->{
\Alert{Estrategias}
\begin{enumerate}
  \visible<12->{\onslide<12, 14->{\item Estrategias puras: siempre se elige la misma acci\'on.}}
  \visible<13->{\onslide<13, 14->{\item Estrategias mixtas: cada acci\'on se elige con cierta probabilidad.}}
\end{enumerate}
}
\end{columns}
...
\Alert{Conceptos}
\begin{enumerate}
  \visible<7->{\onslide<7,18->{\item Ganancia Esperada}}
  \visible<8->{\onslide<8-10,18->{\item Mejor Respuesta}}
  \visible<11->{\onslide<11-15,18->{\item Equilibrio de Nash}}
  \visible<16->{\onslide<16->{\item Equilibrio Correlacionado}}
\end{enumerate}

\column{.5\textwidth}

\only<7>{Valor promedio que un determinado jugador obtendr\'ia si jugara infinitas veces cuando cada jugador utiliza una estrategia dada.}
...
\end{itemize}
\begin{enumerate}
  \visible<2->{\onslide<2,8->{\item Se juega de forma repetida a trav\'es del tiempo $t = 1, 2, 3, ...$.}}
  \visible<3->{\onslide<3,8->{\item A tiempo $t+1$ cada jugador elige una acci\'on siguiendo una estrategia mixta determinada.}}
  \visible<4->{\onslide<4,8->{\item La \textbf{estrategia emp\'irica} converge a un equilibrio de Nash.}}
\end{enumerate}
\vspace{.25cm}
\visible<5->{
...
\begin{enumerate}
  ...
  \visible<6->{\onslide<6,10->{\item Vector invariante de probabilidad de la matriz\\de regret condicional.}}
  \visible<7->{\onslide<7->{\item Regret incondicional.}}
\end{enumerate}
...
\Alert{Observaciones}
\begin{enumerate}
  \visible<2->{\onslide<2,9->{\item Las probabilidades son elegidas proporcional a los regrets positivos.}}
  \visible<3->{\onslide<3,9->{\item El regret va a cero cuando el n\'umero de juegos va a infinito.}}
  \visible<4->{\onslide<4,9->{\item Supongamos que el regret incondicional de cualquier acci\'on es menor que $\frac{\varepsilon}{2} > 0$.}}
  \begin{itemize}
    \visible<5->{\onslide<5,9->{\item La estrategia emp\'irica es una aproximaci\'on a un equilibrio de Nash que se encuentra una distancia no mayor que $\varepsilon$.}}
    \visible<6->{\onslide<6,9->{\item $\varepsilon$-equilibrio de Nash.}}
  \end{itemize}
\end{enumerate}
\vspace{.3cm}

\visible<7->{\Alert{Estrategia Emp\'irica}}\\
\visible<8->{La probabilidad de que un determinado jugador elija una acci\'on $a$ es igual a:
...
\begin{enumerate}
  \visible<2->{ \onslide<2, 11->{\item $4$ juegos para dos jugadores de suma cero.}}
  \visible<3->{ \onslide<3, 11->{\item $10$ corridas por cada uno de los juegos y cada uno de los procedimientos.}}
  \visible<4->{ \onslide<4, 11->{\item Criterio de parada: regret incondicional menor que $0.005$.}}
  \visible<5->{ \onslide<5-10, 11->{\item Tabla de resultados}}
  \begin{itemize}
    \visible<6->{ \onslide<6-10, 11->{\item Ganancia esperada del primer jugador de la \'ultima estrategia obtenida.}}
    \visible<7->{ \onslide<7-10, 11->{\item Explotabilidad de la \'ultima estrategia obtenida.}}
    \visible<8->{ \onslide<8-10, 11->{\item Tiempo promedio.}}
    \visible<9->{ \onslide<9-10, 11->{\item N\'umero de iteraciones promedio.}}
    \visible<10->{ \onslide<10, 11->{\item Promedio del tiempo por iteraci\'on.}}
  \end{itemize}
\end{enumerate}
\end{frame}
...
\visible<2->{\Alert{Valor del juego ($u$):} $0$.}
\vspace{.3cm}
\visible<3->{%
...
\begin{tabular}{l r r r}
    ...
    \onslide<3,4,9->{Ganancia esperada $u(\sigma)$ & $0,000$ & $0,000$ & $0,000$} \\
    \onslide<3,5,9->{Explotabilidad $\varepsilon_{\sigma}$ & $0,006$ & $0,006$ & $0,008$} \\
    \onslide<3,6,9->{Tiempo $T$ & $10,276$ & $0,777$ & $0,042$} \\
    \onslide<3,7,9->{Iteraciones $I$ & $3.892.550,4$ & $25.616,6$ & $16.260,5$}   \\
    \onslide<3,8,9->{$T/I$ & $2,64{\times}10^{-6}$ & $3,03{\times}10^{-5}$ & $2,58{\times}10^{-6}$} \\
    \bottomrule
\end{tabular}
} \end{center}
}
\visible<9->{\graphicsRM{matching-pennies}}
\end{frame}
...
\begin{tabular}{l r r r}
    \toprule
    & \multicolumn{1}{c}{A} & \multicolumn{1}{c}{B} & \multicolumn{1}{c}{C} \\ \midrule
    \onslide<3,4,9->{Ganancia esperada $u(\sigma)$ & $-0,000012$ & $0,000004$ & $0,000022$} \\
    \onslide<3,5,9->{Explotabilidad $\varepsilon_{\sigma}$ &  $0,006$ & $0,010$ & $0,009$} \\
    \onslide<3,6,9->{Tiempo $T$ & $12,198$ & $0,345$ & $0,049$} \\
    \onslide<3,7,9->{Iteraciones $I$ & $4.519.054,1$ & $6.601,3$ &  $19.321,1$} \\
    \onslide<3,8,9->{$T/I$ & $2,70{\times}10^{-6}$ & $5,23{\times}10^{-5}$ & $2,54{\times}10^{-6}$} \\
    \bottomrule
\end{tabular}
} \end{center}
}
\visible<9->{\graphicsRM{RPS}}
\end{frame}
...
\begin{tabular}{l r r r}
    \toprule
    & \multicolumn{1}{c}{A} & \multicolumn{1}{c}{B} & \multicolumn{1}{c}{C} \\ \midrule
    \onslide<2,3,8->{Ganancia esperada $u(\sigma)$ & $0,333$ & $0,334$ & $0,334$}  \\
    \onslide<2,4,8->{Explotabilidad $\varepsilon_{\sigma}$ & $0,010$ & $0,007$ & $0,004$} \\
    \onslide<2,5,8->{Tiempo $T$ & $319,179$ & $11,275$ & $0,237$}  \\
    \onslide<2,6,8->{Iteraciones $I$ & $108.319.272,4$ & $75.250,2$ & $84.318,5$} \\
    \onslide<2,7,8->{$T/I$ & $2,95 {\times} 10^{-6}$ & $1,50 {\times} 10^{-4}$ & $2,81 {\times} 10^{-6}$} \\
    \bottomrule
\end{tabular}
} \end{center}
}
\visible<8->{\graphicsRM{domino}}
\end{frame}
...
\begin{tabular}{l r r r}
    \toprule
    & \multicolumn{1}{c}{A} & \multicolumn{1}{c}{B} & \multicolumn{1}{c}{C} \\ \midrule
    \onslide<2,3,8->{Ganancia esperada $u(\sigma)$ & $0,000219$ & $0,000150$ & $0,000024$} \\
    \onslide<2,4,8->{Explotabilidad $\varepsilon_{\sigma}$ & $0,010$ & $0,010$ & $0,009$}  \\
    \onslide<2,5,8->{Tiempo $T$ &  $875,533$ &  $70,453$ & $0,166$} \\
    \onslide<2,6,8->{Iteraciones $I$ &  $190.222.305,3$ & $58.794,4$  & $48.613,5$} \\
    \onslide<2,7,8->{$T/I$ & $4,60{\times}10^{-6}$ & $1,20{\times}10^{-3}$ & $3,41{\times}10^{-6}$} \\
    \bottomrule
\end{tabular}
} \end{center}
}
\visible<8->{\graphicsRM{blotto-game}}
\end{frame}
...
\begin{columns}
\column{.5\textwidth}
\visible<2->{\Alert{Juegos secuenciales}}
\vspace{.5cm}
...
\begin{enumerate}
  \visible<9->{\onslide<9,18->{\item Historias o nodos.\\Ej: $\emptyset$, $LA, LBr, R$.}}
  \visible<10->{\onslide<10-11,18->{\item Funci\'on que asigna a cada historia (nodo) un jugador.}}
  \visible<11->{
    \begin{itemize}
      \onslide<11,18->{\item Nodos de Azar}
    \end{itemize}
  }
  \visible<12->{\onslide<12-14,18->{\item Conjuntos de informaci\'on.}}
  \visible<15->{\onslide<15-16,18->{\item Funci\'on que asigna por cada historia (nodo) terminal y cada jugador una utilidad.}}
...
(L, $l$) & $0, 0$ & $2, 4$ & $2, 2$ \\ \cline{2-4}
(L,\raisebox{-5pt}{\tikzmark{pA}} $r$) &  $2, 4$ & $0, 0$ & $2, 2$ \\ \cline{2-4}
(R, $l$) & $1, 1$ & $1, 1$ & $1, 1$ \\ \cline{2-4}
(R, $r$) & $1, 1$ & $1, 1$ & $1, 1$ \\ \cline{2-4}
\end{tabular}}
...
\begin{enumerate}
  \visible<4->{\onslide<4-6,14->{\item Estrategias Puras.}}
  \visible<8->{\onslide<8-10,14->{\item Estrategias Mixtas.}}
  \visible<9->{\onslide<9-10,14->{\begin{tabular}{@{}cccc@{}}
  \cmidrule{1-4}
  (L, $l$) & (L, $r$) & (R, $l$) & (R, $r$) \\
  $0.45$ & $0.30$ & $0.00$ &  $0.25$ \\ \cmidrule{1-4}
  \end{tabular}}}
  \visible<10->{\onslide<10,14->{\begin{tabular}{@{}ccc@{}}
  \cmidrule{1-3}
  A & B & C \\
  $0.25$ & $0.25$ & $0.50$ \\
  \cmidrule{1-3}
  \end{tabular}}}
  \visible<11->{\onslide<11-12,14->{\item Estrategias de Comportamiento.}}
  \visible<12->{\onslide<12,14->{\begin{tabular}{@{}cccc@{}}
...
Cada jugador utiliza una mejor respuesta frente a su oponente.}
\end{columns}
\end{frame}
...
\begin{enumerate}[--]
  ...
  \visible<5->{\item ?`Por qu\'e es mejor utilizar una estrategia no determinista?}
  \visible<6->{\item ?`Es posible ``enga\~nar'' al oponente?}
\end{enumerate}
\end{frame}

\begin{frame}{Counterfactual Regret Minimization (CFR)}
\begin{enumerate}
  \visible<2->{\onslide<2,10->{\item El \'arbol del juego se recorre repetidamente. La m\'aquina juega contra s\'i misma de forma repetida.}}
  \visible<3->{\onslide<3,10->{\item Se inicia con distribuci\'on uniforme en cada conjunto de informaci\'on.}}
  ...
  \visible<5->{\onslide<5,10->{\item La estrategia promedio converge a un equilibrio de Nash.}}
\end{enumerate}
\vspace{.25cm}

\visible<6->{\Alert{?`C\'omo se mejora la estrategia en cada iteraci\'on?}}
\begin{enumerate}[--]
\visible<7->{\onslide<7,10->{\item Sumar el regret (arrepentimiento) que se tiene en cada conjunto de informaci\'on por cada acci\'on.}}
\visible<8->{\onslide<8,10->{\item \textbf{Regret}: cu\'anto mejor lo habr\'ia hecho en todos los juegos hasta ahora si siempre hubiera jugado esta acci\'on en este conjunto de informaci\'on.}}
...
\begin{enumerate}[--]
  \visible<2->{\onslide<2,15->{\item Tres clases de juegos, cada uno con diferentes par\'ametros.}}
  \visible<3->{\onslide<3,15->{\item CFR con muestreo en los nodos de azar.}}
  \visible<4->{\onslide<4,15->{\item 10 horas de entrenamiento.}}
  \visible<5->{\onslide<5,15->{\item Un juego se considera resuelto si la explotabilidad es menor que el $1\%$ de la m\'inima ganancia positiva posible.}}
  \visible<6->{\onslide<6,15->{\item Gr\'afica del regret con respecto al n\'umero de iteraciones.}}
  \visible<7->{\item Tabla de resultados:}
    \begin{columns}%
    \column{.45\textwidth}%
    \begin{itemize}
...
\resizebox{\textwidth}{!}{\begin{tabular}{@{}ccc@{}}
  {\includegraphics{../graphics/cfr/ocp/OCP(3).png}} &
  {\includegraphics{../graphics/cfr/ocp/OCP(12).png}} &
...
\begin{frame}{\Large Dudo}
\begin{center}
\resizebox{!}{1.05cm}{\begin{tabular}{@{}cccc@{}}
  {\includegraphics{../graphics/cfr/dudo/Dudo(4,1,1).png}} &
  {\includegraphics{../graphics/cfr/dudo/Dudo(4,1,2).png}} &
  {\includegraphics{../graphics/cfr/dudo/Dudo(4,2,1).png}} &
  {\includegraphics{../graphics/cfr/dudo/Dudo(4,2,2).png}}
\end{tabular}}
\vspace{.3cm}

\resizebox{!}{1.05cm}{\begin{tabular}{@{}cccc@{}}
  {\includegraphics{../graphics/cfr/dudo/Dudo(5,1,1).png}} &
  {\includegraphics{../graphics/cfr/dudo/Dudo(5,1,2).png}} &
  {\includegraphics{../graphics/cfr/dudo/Dudo(5,2,1).png}} &
  {\includegraphics{../graphics/cfr/dudo/Dudo(5,2,2).png}}
\end{tabular}}
\vspace{.3cm}

\resizebox{!}{1.05cm}{\begin{tabular}{@{}ccc@{}}
  {\includegraphics{../graphics/cfr/dudo/Dudo(6,1,1).png}} &
...
\begin{enumerate}
  \visible<3->{\onslide<3,10->{\item Estudio de juegos no determinista con informaci\'on incompleta: forma normal y forma extensiva.}}
  \visible<4->{\onslide<4,10->{\item Equilibrio de Nash como concepto de soluci\'on principal en los juegos de dos jugadores de suma cero.}}
  \visible<5->{\onslide<5,10->{\item Algoritmos de Regret Matching y Counterfactual Regret Minimization para encontrar aproximaciones de equilibrios de Nash.}}
  \visible<6->{\onslide<6,10->{\item Explotabilidad como m\'etrica de error en las estrategias encontradas.}}
\end{enumerate}
\vspace{.25cm}

\visible<7->{\Alert{Recomendaciones}}
\begin{enumerate}
  \visible<8->{\onslide<8,10->{\item Resolver instancias mayores del juego de domin\'on para $2$ personas considerando abstracciones.}}
  \visible<9->{\onslide<9,10->{\item Experimentos sobre el juego para $4$ personas considerando cada pareja como un \'unico jugador.}}
\end{enumerate}
\end{frame}
