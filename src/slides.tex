...
\begin{enumerate}[--]
  ...
  \visible<3->{\item Valor del juego ($u$): ganancia del primer jugador cuando ambos jugadores utilizan un equilibrio de Nash.}
  \visible<4->{\item El juego ``batalla de los sexos'' no cumple estas condiciones.}
  % \begin{itemize}
...
\begin{enumerate}
  \visible<3->{\onslide<3-5>{\item Regret condicional.}}
  \visible<6->{\onslide<6-8>{\item Regret incondicional.}}
...
\begin{enumerate}
  ...
  \visible<7->{\item Verificaci\'on: la explotabilidad mide la distancia entre la estrategia actual y un equilibrio de Nash.}
...
\begin{enumerate}
  \visible<2->{\item El equilibrio de Nash es un concepto de soluci\'on satisfactorio en juegos en forma normal de dos jugadores de suma cero.}
  \visible<3->{\item Los algoritmos de Regret Matching permiten encontrar aproximaciones de un equilibrio de Nash en este tipo de juegos.}
  \visible<4->{\item El procedimiento ``regret condicional'' necesita mayor n\'umero de iteraciones, y por lo tanto mayor tiempo, para converger.}
  \visible<5->{\item El procedimiento ``vector invariante de probabilidad'' es el procedimiento con las iteraciones m\'as costosas.}
  \visible<6->{\item El procedimiento ``regret incondicional'' es el m\'as eficiente.}
  \visible<7->{\item Si el regret incondicional es menor a 0,005 por jugador, la estrategia emp\'irica tiene una explotabilidad no mayor que 0,01.}
  \visible<8->{\item Fue posible resolver diversos juegos en forma normal.}
\end{enumerate}
\end{frame}
...
\visible<2->{
\node[player1] (root) {}
  child { [onslide=<2>{color=white}] node (L) [player2, onslide=<2>{color=white}]  {}
      child { [onslide=<2-3>{color=white}] node (A) [player1, onslide=<2-3>{color=white}]  {}
            child {[onslide=<2-4>{color=white}] node [terminal, onslide=<2-4>{color=white}] [label=below:{$(0,0)$}] {}
                edge from parent node[left] {$a$} }
            child {[onslide=<2-4>{color=white}] node [terminal, onslide=<2-4>{color=white}] [label=below:{$(2,4)$}] {}
                edge from parent node[right] {$b$}}
            edge from parent node[left] {A\, \, }
      }
      child { [onslide=<2-3>{color=white}] node (B) [player1, onslide=<2-3>{color=white}] {}
          child {[onslide=<2-4>{color=white}] node (t1) [terminal, onslide=<2-4>{color=white}] [label=below:{$(2,4)$}] {}
              edge from parent node[left] {$a$} }
          child {[onslide=<2-4>{color=white}] node (t2) [terminal, onslide=<2-4>{color=white}] [label=below:{$(0,0)$}] {}
              edge from parent node[right] {$b$} }
          edge from parent node[right] {B}
      }
      child { [onslide=<2-3>{color=white}] node (C) [chance, onslide=<2-3>{color=white}] {}
          child {[onslide=<2-5>{color=white}] node (t3) [terminal, onslide=<2-5>{color=white}] [label=below:{$(4,4)$}] {}
              edge from parent node[left] {$\visible<16>{\frac{1}{2}}\ c$} }
          child {[onslide=<2-5>{color=white}] node (t4) [terminal, onslide=<2-5>{color=white}] [label=below:{$(0,0)$}] {}
              edge from parent node[right] {$s\ \visible<16>{\frac{1}{2}}$} }
          edge from parent node[right] {\, \, C}
      }
      edge from parent node[left] [label=left:{L}] {}
  }
  child { [onslide=<2>{color=white}] node (R) [terminal, onslide=<2>{color=white}] [label=below:{$(1,1)$}] {}
      edge from parent node[right] [label=right:{R}] {}
  }
;}
\visible<4->{\draw[dotted] (A) -- (B);}

\visible<9>{\draw[thick] (C) ellipse (.6cm and .6 cm);}
\visible<11>{\draw[thick, blue] ($(A)!0.5!(B)$) ellipse (2cm and .65cm);}
\visible<11>{\draw[thick, blue] (root) ellipse (.6cm and .6 cm);}
\visible<12>{\draw[thick, red] (L) ellipse (.6cm and .6 cm);}

\node (c1) at ($(t1)!0.5!(t2)$) {};
\node (c2) [below of=c1] {};
\node (c0) at ($(c1)!0.3!(c2)$) {};
\visible<14>{\draw[dashed] (c0) ellipse (4cm and .75cm);}
\node (R1) [below of=R] {};
\node (R0) at ($(R)!0.3!(R1)$) {};
\visible<14>{\draw[dashed] (R0) ellipse (.75cm and .6cm);}
\visible<16>{\draw[dashed] ($(t3)!0.5!(t4)!0.25!(C)$) ellipse (1.5cm and 1.5cm);}
\end{tikzpicture}
}}

\vspace{.1cm}
\visible<17->{\Alert{Perfect Recall}\\
Se recuerda de forma perfecta todo lo que se ha visto.}

\column{.45\textwidth}
\visible<7->{\Alert{Elementos}}

\begin{enumerate}
  \visible<7->{\onslide<7>{\item Historias o nodos.\\Ej: $\emptyset$, $LA, LBb, R$.}}
  \visible<8->{\onslide<8-9>{\item Funci\'on que asigna a cada historia (nodo) no terminal un jugador.}}
  \visible<9->{
    \begin{itemize}
      \onslide<9>{\item Nodos de azar.}
    \end{itemize}
  }
  \visible<10->{\onslide<10-12>{\item Conjuntos de informaci\'on.}}
  \visible<13->{\onslide<13-14>{\item Funci\'on que asigna por cada historia (nodo) terminal y cada jugador una utilidad.}}
  \visible<15->{\onslide<15-16>{\item Distribuci\'on de probabilidad sobre el conjunto de acciones en cada nodo de azar.}}
\end{enumerate}
\end{columns}
\end{frame}
...
\begin{enumerate}
  \visible<2->{\item El \'arbol del juego se recorre repetidamente.} %La m\'aquina juega contra s\'i misma a lo largo del tiempo.}}
  \visible<3->{\item Inicia con una estrategia uniforme en cada conjunto de informaci\'on.}
  \visible<4->{\item En cada iteraci\'on se elige una mejor estrategia revisando las decisiones pasadas, utilizando las m\'etricas de regret.}
  \visible<5->{\item La estrategia promedio converge a un equilibrio de Nash; en la pr\'actica el algoritmo es detenido despu\'es de cierto n\'umero de iteraciones.}
\end{enumerate}
\visible<6->{\Alert{?`C\'omo se mejora la estrategia en cada iteraci\'on?}}
\begin{enumerate}[--]
\visible<7->{\item Sumar el regret contrafactual que se tiene en cada conjunto de informaci\'on por cada acci\'on.}
\visible<8->{\item \textbf{Regret contrafactual}: cu\'anto mejor lo habr\'ia hecho en todos los juegos hasta ahora si siempre hubiera jugado esta acci\'on en este conjunto de informaci\'on.}
\visible<9->{\item \textbf{Regret Matching}: en la nueva estrategia las acciones son elegidas con probabilidades proporcionales a los regrets positivos.}
\end{enumerate}
\end{frame}

\begin{frame}{\Large CFR: Experimentos}
\begin{enumerate}
  \visible<2->{\item CFR con muestreo en los nodos de azar.}
  \visible<3->{\item Implementaci\'on propia del algoritmo.}
  ...
  \end{itemize}}
  \visible<5->{\item Tres clases de juegos, cada uno con diferentes par\'ametros.}
  \visible<6->{\item 10 horas de entrenamiento.}
  \visible<7->{\item Gr\'afica del regret con respecto al n\'umero de iteraciones.}
  \visible<8->{\item Verificaci\'on: la explotabilidad mide la distancia entre la estrategia actual y un equilibrio de Nash.}
  \visible<9->{\item Un juego se considera resuelto si la explotabilidad es menor que el $1\%$ de la m\'inima ganancia positiva posible.}
    % \begin{columns}%
    % \column{.45\textwidth}%
    % \begin{itemize}
...
\resizebox{.95\textwidth}{!}{\begin{tabular}{lrrrrrc}
    \toprule
    Juego & $N$ & $I$ & Iteraciones & $u(\sigma)$ & $\varepsilon_{\sigma}$ (\%) & Resuelto \\ \midrule
    \onslide<6>{Dudo$(4, 1, 1)$ &         8.177 &         512 & 18.697.532 & -0,125 &  0,0259 & \cmark} \\
    \onslide<6>{Dudo$(4, 1, 2)$ &       327.641 &      14.366 &  1.215.600 & -0,508 &  0,0971 & \cmark} \\
    \onslide<6>{Dudo$(4, 2, 1)$ &       327.641 &      14.366 &  1.213.799 &  0,552 &  0,3701 & \cmark} \\
    Dudo$(4, 2, 2)$ &    13.107.101 &      327.680 &     63.109 & 0,0069 &  2,1132 & \xmark \\
    \onslide<6>{Dudo$(5, 1, 1)$ &        51.176 &       2.560 &  4.521.208 & -0,120 &  0,1186 & \cmark} \\
    \onslide<6>{Dudo$(5, 1, 2)$ &     4.915.126 &     163.840 &    151.235 & -0,565 &  0,6197 & \cmark} \\
    \onslide<6>{Dudo$(5, 2, 1)$ &     4.915.126 &     163.840 &    143.698 &  0,581 &  0,0122 & \cmark} \\
    Dudo$(5, 2, 2)$ &   471.858.976 &   7.864.320 &      3.826 &  0,836 & 15,1963 & \xmark \\
    \onslide<6>{Dudo$(6, 1, 1)$ &       294.877 &      12.288 &  1.067.782 & -0,111 &  0,0975 & \cmark} \\
...
\resizebox{.95\textwidth}{!}{\begin{tabular}{lrrrrrc}
    \toprule
    Juego & $N$ & $I$ & Iteraciones & $u(\sigma)$ & $\varepsilon_{\sigma}$ (\%) & Resuelto \\ \midrule
    \onslide<6>{Domino$(2, 2)$ &         7.321 &     102 & 540.186.366 & 2,4000 & 0,0000 & \cmark} \\
    \onslide<6>{Domino$(3, 2)$ &    46.534.657 &  88.947 & 400.047.334 & 2,8767 & 0,0315 & \cmark} \\
    \onslide<6>{Domino$(3, 3)$ &   246.760.993 & 107.854 &  72.492.951 & 2,1539 & 0,3854 & \cmark} \\
...
\begin{enumerate}[--]
  \visible<2->{\item Juegos no resueltos con $10$ horas de entrenamiento.}
  \visible<3->{\item $200$ horas de entrenamiento.}
\end{enumerate}
...
\resizebox{.95\textwidth}{!}{\begin{tabular}{lrrrrrc}
    \toprule
    Juego & $N$ & $I$ & Iteraciones & $u(\sigma)$ & $\varepsilon_{\sigma}$ (\%) & Resuelto \\ \midrule
    \onslide<4>{Dudo$(4, 2, 2)$ &    13.107.101 &      327.680 &    2.276.259 &  0,00875 & 0,2382 & \cmark} \\
    Dudo$(5, 2, 2)$ &                   471.858.976 &    7.864.320 &      133.863 & -0,00004 & 1,7695 & \xmark  \\
    \onslide<4>{Dudo$(6, 1, 2)$ &    66.060.163 &    1.769.472 &      543.485 &   -0,597 & 0,5102 & \cmark} \\
    \onslide<4>{Dudo$(6, 2, 1)$ &    66.060.163 &    1.769.472 &      513.786 &    0,597 & 0,6727 & \cmark} \\
    \onslide<4>{Domino$(3, 4)$  & 1.547.645.185 &      104.050 &  365.484.932 &   3.2027 & 0,1812 & \cmark} \\
    \bottomrule
\end{tabular}}
\end{center}}
\end{frame}
...
\begin{enumerate}
  \visible<2->{\item Los modelos utilizados son adecuados para los juegos planteados.}
  \visible<3->{\item El equilibrio de Nash es un concepto de soluci\'on satisfactorio en juegos de dos jugadores de suma cero. No lo es cuando el juego no es de suma cero o tiene m\'as de dos jugadores.}
  \visible<4->{\item Se utiliz\'o la explotabilidad como m\'etrica para medir la distancia entre la estrategia obtenida y un equilibrio de Nash.}
  \visible<5->{\item Resoluci\'on de juegos: se encontraron aproximaciones con una explotabilidad no mayor que el $1\%$ de la m\'inima ganancia positiva posible.}
\end{enumerate}

\visible<6->{\Alert{Recomendaciones}}
\begin{enumerate}
  \visible<6->{\item Resolver instancias mayores del juego de domin\'o para $2$ personas considerando abstracciones.}
  \visible<7->{\item Experimentos sobre el juego para $4$ personas considerando cada pareja como un \'unico jugador.}
\end{enumerate}
\end{frame}
...
\visible<2->{%
\Alert{\only<2-5>{Equilibrio de Nash $\sigma^* = (\sigma^*_1, \sigma^*_2)$}\only<6->{Aproximaci\'on $\sigma' = (\sigma'_1, \sigma'_2)$}}
\visible<3-5, 7->{%
\begin{enumerate}
  \visible<3-5, 7->{\item Primer jugador garantiza una ganancia esperada de \\ al menos \only<3-5>{$u$}\only<7->{$u-\varepsilon_1$}.}
  \visible<4-5, 9->{\item Segundo jugador garantiza una ganancia esperada de \\ \only<4>{al menos $-u$}\only<5, 9->{a lo sumo }\only<5>{$u$}\only<9->{$u+\varepsilon_2$}\only<5, 9->{ para el primer jugador}.\visible<0>{fixheight}}
\end{enumerate}}
}

\begin{center}
  \begin{tikzpicture}
  \visible<2->{\draw[<->, very thick] (-5.25, 0) -- (5.25, 0);}
  \visible<2->{\draw[dashed] (0, 1.5) -- (0, -1);}
  \visible<2->{\node[align=center] at (0.15, 0.2) {\small{$u$}};}
  \visible<2->{\draw[fill=black] (0, 0) circle (0.05cm);}
  \visible<3>{\draw[->, rounded corners, blue, very thick](0, 0) -- (0, -0.6) -- (5.25, -0.6);}
  \visible<3->{\draw[->, rounded corners, blue, very thin](0, 0) -- (0, -0.6) -- (5.25, -0.6);}
  \visible<3->{\node[text width=3.5cm] at (3.5,-1) {$u_1(\sigma_1^*, \sigma_2) \geq u $};}
  \only<4>{\node[text width=3.5cm] at (-3,-1) {$u_2(\sigma_1, \sigma^*_2) \geq -u $};}
  \visible<4->{\draw[->, rounded corners, red, very thin](0, 0) -- (0, -0.5) -- (-5.25, -0.5);}
  \visible<4-5>{\draw[->, rounded corners, red,  very thick](0, 0) -- (0, -0.5) -- (-5.25, -0.5);}
  \visible<5->{\node[text width=3.5cm] at (-3,-1) {$u_1(\sigma_1, \sigma^*_2) \leq u $};}
  \visible<7->{\draw[<->](-0.01, 1) -- (-1.5, 1);}
  \visible<7->{\draw[dashed] (-1.5, 2) -- (-1.5, -0.45);}
  \visible<7->{\node[text width=1cm] at (-0.4, 1.25) {$\varepsilon_1$};}
  \visible<7->{\node[align=left] at (-0.9, 0.2) {\small{$u-\varepsilon_1$}};}
  \visible<7->{\draw[fill=black] (-1.5, 0) circle (0.05cm);}
  \visible<8->{\draw[->, rounded corners, blue, very thin](-1.5, 0) -- (-1.5, 0.6) -- (5.25, 0.6);}
  \visible<8>{\draw[->, rounded corners, blue, very thick](-1.5, 0) -- (-1.5, 0.6) -- ( 5.25, 0.6);}
  \visible<8->{\node[text width=3.5cm] at (3.5, 1) {$u_1(\sigma'_1, \sigma_2) \geq u - \varepsilon_1$};}
  \visible<9->{\draw[<->](0.01, 1) -- (0.8, 1);}
  \visible<9->{\draw[dashed] (0.8, 2) -- (0.8, -0.45);}
  \visible<9->{\node[text width=1cm] at (0.7, 1.25) {$\varepsilon_2$};}
  \visible<9->{\node[align=left] at (1.35, 0.2) {\small{$u+\varepsilon_2$}};}
  \visible<9->{\draw[fill=black] (0.8, 0) circle (0.05cm);}
  \visible<10->{\draw[->, rounded corners, red, very thin](0.8, 0) -- (0.8, 0.5) -- (-5.25, 0.5);}
  \visible<10>{\draw[->, rounded corners, red,  very thick]( 0.8, 0) -- ( 0.8, 0.5) -- (-5.25, 0.5);}
  \visible<10->{\node[text width=3.5cm] at (-3.25, 1) {$u_1(\sigma_1, \sigma'_2) \leq u + \varepsilon_2$};}
  \visible<11->{\draw[<->](-1.5, 1.7) -- (0.8, 1.7);}
  \visible<11->{\node[text width=1cm] at (0.1, 2) {$\varepsilon$};}
  \end{tikzpicture}

\visible<11->{\Alert{{\Large \textbf{$\varepsilon = \varepsilon_1 + \varepsilon_2$}}}}
\end{center}
\end{frame}
...
Dos prisionerso son sospechosos de un crimen y se interrogan en celdas separadas.
\begin{enumerate}[--]
...
\begin{frame}{\Large N Jugadores}

\RedAlert{El tercer jugador siempre pierde, pero su decisi\'on afecta las ganancias de los otros $2$ jugadores.}
\end{frame}
