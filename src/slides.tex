...
\node[player1] (root) {}
  child { [onslide=<3>{color=white}] node (L) [player2, onslide=<3>{color=white}]  {}
      ...
      child { [onslide=<3-4>{color=white}] node (C) [chance, onslide=<3-4>{color=white}] {}
          child {[onslide=<3-6>{color=white}] node (t3) [terminal, onslide=<3-6>{color=white}] [label=below:{$(3,3)$}] {}
...
\begin{enumerate}
  \visible<9->{\item Historias o nodos.\\Ej: $\emptyset$, $LA, LBr, R$.
      \begin{itemize}
      \item Acciones
    \end{itemize}
  }
  \visible<10->{\item Funci\'on que asigna a cada historia (nodo) un jugador.}
  \visible<11->{
    \begin{itemize}
      \item Nodos de Azar
    \end{itemize}
  }
  \visible<12->{\item Conjuntos de informaci\'on.}
  \visible<15->{\item Funci\'on que asigna por cada historia (nodo) terminal y cada jugador una utilidad.}
  \visible<17->{\item Distribuci\'on de probabilidad sobre el conjunto de acciones en cada nodo de azar.}
\end{enumerate}
\end{columns}
\end{frame}
