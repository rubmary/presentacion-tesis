...
\Alert{Observaciones}
\begin{enumerate}
  \visible<2->{\onslide<2,7->{\item Las probabilidades son elegidas proporcional a los regrets positivos.}}
  \visible<3->{\onslide<3,7->{\item El regret positivo tiende a cero cuando el n\'umero de juegos tiende a infinito.}}
  \visible<4->{\onslide<4,7->{\item Si el regret positivo es peque\~no, la \textbf{estrategia emp\'irica} es una aproximaci\'on a un equilibrio de Nash.}}
\end{enumerate}
\vspace{.3cm}

\visible<5->{\Alert{Estrategia Emp\'irica}}\\
\visible<6->{La probabilidad de que un determinado jugador elija una acci\'on $a$ es igual a:
...
\begin{frame}{\Large Experimentos}

\begin{enumerate}
  \visible<2->{ \onslide<2, 12->{\item $4$ juegos para dos jugadores de suma cero.}}
  \visible<3->{ \onslide<3, 12->{\item $10$ corridas por cada uno de los juegos y cada uno de los procedimientos.}}
  \visible<4->{ \onslide<4, 12->{\item Criterio de parada: regret incondicional menor que $0.005$.}}
  \visible<5->{ \onslide<5, 12->{\item Comprobaci\'on con un problema equivalente de programaci\'on lineal.}}%
  \visible<6->{ \onslide<6->{\item Tabla de resultados:}}%
  \begin{itemize}%
    \visible<7->{ \onslide<7->{\item Ganancia esperada del primer jugador de la \'ultima estrategia obtenida.}}
    \visible<8->{ \onslide<8->{\item Explotabilidad de la \'ultima estrategia obtenida.}}
    \visible<9->{ \onslide<9->{\item Tiempo promedio.}}
    \visible<10->{ \onslide<10->{\item N\'umero de iteraciones promedio.}}
    \visible<11->{ \onslide<11->{\item Promedio del tiempo por iteraci\'on.}}
  \end{itemize}
\end{enumerate}
\end{frame}
...
\visible<8->{\onslide<8,10->{\item \textbf{Regret} (de una acci\'on en un conjunto de informaci\'on): cu\'anto mejor lo habr\'ia hecho en todos los juegos hasta ahora si siempre hubiera jugado esta acci\'on en este conjunto de informaci\'on.}}
