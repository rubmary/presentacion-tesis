...
\Alert{Elementos}
\begin{enumerate}
  \onslide<7, 14->{\item Jugadores.}
  \onslide<8, 14->{\item Acciones o estrategias puras: $\mathcal{R}$, $\mathcal{P}$, $\mathcal{S}$.}
  \onslide<9, 14->{\item Funci\'on de pago o utilidades.}
\end{enumerate}
}
\column{.52\textwidth}
\visible<11->{
\Alert{Estrategias}
\begin{enumerate}
  \onslide<12, 14->{\item Estrategias puras: siempre se elige la misma acci\'on.}
  \onslide<13, 14->{\item Estrategias mixtas: cada acci\'on se elige con cierta probabilidad.}
\end{enumerate}
}
\end{columns}
...
\begin{enumerate}
  ...
  \begin{itemize}
    \visible<5->{\item La estrategia emp\'irica es una aproximaci\'on a un equilibrio de Nash que se encuentra una distancia no mayor que $2\varepsilon$.}
    \visible<6->{\item $2\varepsilon$-equilibrio de Nash.}
  \end{itemize}
\end{enumerate}
\vspace{.3cm}
...
\visible<8->{\onslide<8,10->{\item \textbf{Regret} Cu\'anto mejor lo habr\'ia hecho en todos los juegos hasta ahora si siempre hubiera jugado esta acci\'on en este conjunto de informaci\'on, en lugar de elegir cualquier combinaci\'on de acciones que mi estrategia dijo que deb\'i usar}}
